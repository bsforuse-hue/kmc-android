...
class KMCAndroidApp(App):
    ...
    def scan_files(self, path):
        self.log_label.text = "" # ניקוי מסך לפני סריקה חדשה
        self.log(f"Analyzing Folder: {path}", "55ffff")
        self.log("- - - - - - - - - -", "cccccc")
        ...
        try:
            ...
            count_rsp_ok = 0
            count_rsp_fail = 0
            error_list = [] # רשימה לשמירת שמות הקבצים שנכשלו
            
            for f_name in files:
                full_path = os.path.join(path, f_name)
                try:
                    with open(full_path, 'rb') as f: data = f.read(64)
                    ...
                    if m_type == 0x41: 
                        ...
                        if len(data) > 26:
                            if data[26] == 0:
                                ...
                                count_rsp_fail += 1
                                error_code = data[26]
                                error_list.append(f"{f_name} (Err {error_code})")
                    else:
                        # כל מה שהוא לא RSP נחשב כמפתח (REQ/KMAC)
                        count_req += 1
                        
                except Exception as e:
                    self.log(f"Error reading {f_name}", "ff0000")

            # --- הדפסת דוח סיכום ---
            self.log(f"Total Keys (REQ/KMAC): {count_req}", "ffff00") # צהוב
            self.log(f"Successful Installs (RSP): {count_rsp_ok}", "00ff00") # ירוק
            
            if count_rsp_fail > 0:
                self.log(f"Failed Installs: {count_rsp_fail}", "ff0000") # אדום
                self.log("--- Failure Details ---", "ff5555")
                for err in error_list:
                    self.log(f" > {err}", "ffaaaa")
            else:
                self.log("No Errors Found.", "00ff00")
